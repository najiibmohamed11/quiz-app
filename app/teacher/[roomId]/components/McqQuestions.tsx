 ...
 const [form, setForm] = useState<z.infer<typeof mcqSchema>>({
    ...
    setError("")
      ...
      const questionId= createMcq({...form,roomId:roomId as Id<'rooms'> })
        ...
        toast.promise(questionId,{
          loading:"Loading....",
          success:()=>{
            setForm({question:"",correctAnswerIndex:undefined,options:['','',''],questionType:"MCQ"})
           setIsLoading(false)
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-4">
      ...
      >
        {form.options.map((option, index) => (
          <div key={index} className="flex items-center gap-3">
            ...
            />
            <Button disabled={form.options.length<3} type="button" variant="ghost" onClick={() => handleRemoveOption(index)}>
              <X />
            </Button>
          </div>
        ))}
      ...
      <Button disabled={form.options.length>=5} type="button" className="w-1/3" onClick={handleAddOption}>
        Add option
      </Button>
        <em className="text-red-500 text-center">{error&&error}</em>
      <Button disabled={isLoading} type="submit">{isLoading?"Adding.....":"Add Question"}</Button>
    </form>
  )
}
