...
import { UniqueColumnDropDown } from './UniqueColumnDropDown';
 ...
 const ImportStudents = () => {
  ...
  const handleFormChange=async(e:ChangeEvent<HTMLInputElement>)=>{
      if(e.target.files&&e.target.files.length>0){  
       ...
       // Get first sheet
        ...
        const rowsData=XLSX.utils.sheet_to_json(sheet,{header:1,defval:"", blankrows:false})as [][]

        const [columns,...rows]=rowsData
        if(columns.length>2){
          setError(`Table you provided is more then 2 columns (${columns}) please profide 2 or less columns. e.g StudentId,StudentName`);
          return ;
        }
        if(rows.length>100){
          setError(`we can handle student more then 100  you table contains ${rows.length}`);
          return ;
        }
  ...
  return (
    <>
    <div className={`flex ${!importedData?'justify-center':'justify-between'} mt-3`}>
         <Input ref={fileInputRef} id="restrict" onChange={handleFormChange} type="file" placeholder="najiib" accept=".xlsx,.xls"  className="bg-green-600 hover:bg-green-700 w-90 hidden"/>
         <Button className='bg-green-700 hover:bg-green-700' onClick={handleFileImport}><Sheet/> import students list</Button>
         {(uniqueColumns.length>0&&importedData)&&<UniqueColumnDropDown unique={uniqueColumns} />}
    </div>
